...
Agentstration already consumes external MCP Tools through governed `ToolProvider` and `Tool` resources, while ADR-0099 requires bounded business capabilities to be invocable by Agents and MCP clients without exposing a generic Flow launcher. Static attributed server methods cannot represent Workspace-authored Tools or follow an active published Flow version.
