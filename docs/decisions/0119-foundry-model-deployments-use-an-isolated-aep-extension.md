...
- The first increment uses the documented project REST API directly, rather than coupling discovery to a project SDK transport that could bypass the extension's outbound address policy. `Azure.Identity` is used for explicit managed, workload or Development credentials. The temporary API-key mode reads `FOUNDRY_API_KEY` from the extension process environment, with a project User Secrets fallback only in the local Development host environment. No credential is accepted in Model Profile options, resource manifests or URL fields.
