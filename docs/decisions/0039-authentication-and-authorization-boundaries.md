...
Password changes and current-user revocation of other cookie sessions are delivered through ASP.NET Core Identity. [ADR-0042](0042-security-events-are-an-append-only-management-log.md) delivers the first durable security audit. Add password recovery and optional confirmation flows only after defining a secure delivery channel. Remaining work includes Platform administrator reassignment, account deletion/retention policy, external-account provisioning/linking, audit retention/export, and authorization coverage for the remaining planes. Reconcile Work/Workplace Workspace identities before applying canonical Workspace policies there.
