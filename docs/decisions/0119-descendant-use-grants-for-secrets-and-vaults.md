...
- Secret and Vault definitions each carry this policy. Management create and update operations validate it before persistence. Existing definitions without a policy deserialize with an empty policy.
...
- The JSON resource payload gains optional policy metadata; SQLite and PostgreSQL relational schemas do not change. Existing workspace-owned Secrets and Vaults continue to resolve in their own scope.
