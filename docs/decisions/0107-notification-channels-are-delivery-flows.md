...
- Agentstration generates an opaque unique notification identity for every creation. The Tool caller supplies only functional notification content and never controls the persisted resource key.
...
The notification record retains causal identifiers but no complete Tool arguments or provider payload. Its identifier is generated by Agentstration for each creation, so identical functional payloads remain distinct notifications.
