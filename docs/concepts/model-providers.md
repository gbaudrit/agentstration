...
When an AEP contribution declares configuration value requirements, the Console binds each requirement to an exact scoped `Parameter` or `Secret`. The Model Provider editor can create either resource in place and selects the returned exact reference automatically. These creation dialogs are shared Console components rather than Model Provider-specific forms, so other resource editors can reuse the same governed flow. A Parameter keeps its visible typed value; a Secret requires an eligible Vault and its value remains write-only. The Console never reads a Secret value back, and server-side scope, grant, type, and format validation still applies when the provider is saved and used.
