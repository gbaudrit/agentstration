...
2. Start the optional extension. Aspire uses `Foundry:Enabled=true` and the AppHost User Secrets described above. Compose uses `base.yml` plus `foundry.yml` with values in the ignored `deploy/compose/.env.foundry` file; see [Compose instructions](../deploy/compose/README.md#sqlite). The Compose example uses the interim environment key. It does not provide an Azure CLI or workload identity to a container. Configure and mount that identity explicitly in the hosting environment if using Entra there.
