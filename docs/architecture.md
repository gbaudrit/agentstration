...
Extension source discovery runs once at instance scope after persistence initialization and before declarative bootstrap, so every Tenant can reference configuration- or Aspire-backed registrations deterministically. An idempotent pass after bootstrap covers an instance initialized by the first profile application. `Agentstration:Extensions:DiscoverOnStartup` defaults to `true` and disables both automatic passes when set to `false`.
