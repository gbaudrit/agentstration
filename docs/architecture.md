...
29. **Delivered Tool governance audit read API:** `GET /api/tool-governance/{runtime|flow}/{runId}` reads the current Workspace's existing durable journal with an `afterSequence` cursor, bounded `limit`, and exact `toolId`, `hookId` and `decision` filters. The safe response contract exposes invocation and policy identities but never Tool arguments, provider results or denial messages.
