...
`Agentstration.Resources` contains the neutral namespace, ownership-scope, and address value types shared across boundaries. Management resources retain globally unique UIDs and use `(scope, namespace, kind, name)` as their exact logical identity. A scope is explicitly instance, tenant, or workspace owned. Existing workspace callers still use their exact ambient workspace and implicitly use the `default` namespace. Relative references inherit their owner's namespace; explicit cross-namespace references retain the supplied namespace. See ADR-0035 and ADR-0079.
...
The Management boundary persists a `Tenant -> Workspace` hierarchy and a global `User -> TenantMembership -> RoleAssignment -> RoleDefinition` authorization model. Management resource rows keep their UID-backed physical identity and carry an explicit immutable ownership scope (`instance`, `tenant`, or `workspace`) in addition to Tenant/Workspace columns and their JSON payload. Exact-scope operations are separate from descendant-visible enumeration: visibility flows only from instance to tenant to that tenant's workspaces, and homonymous resources are never merged, shadowed, or overridden. Request contexts constrain reads and writes to authorized scope ancestry; unrestricted system enumeration remains explicit. This generic support does not make every resource kind creatable at every scope. See ADR-0079.
