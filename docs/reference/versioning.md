...
Agentstration uses Semantic Versioning: `MAJOR.MINOR.PATCH`, with prereleases such as `0.2.0-alpha.1`, `0.2.0-beta.1`, and `0.2.0-rc.1`. During `0.x`, public contracts remain under development.
...
The Source Registry .NET tool is version-aligned with the product rather than autonomous: it implements the Source contracts accepted by that release. The same `v<version>` tag starts its dedicated package workflow. A product prerelease produces a NuGet prerelease package, while a product version without a prerelease suffix produces a stable package.
