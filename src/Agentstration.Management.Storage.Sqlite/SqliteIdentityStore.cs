...
internal sealed class ResourceGroupRow { public Guid Id { get; set; } public Guid TenantId { get; set; } public Guid WorkspaceId { get; set; } public required string Name { get; set; } public DateTimeOffset CreatedAt { get; set; } }
...
internal static class IdentityModel
{
    public static void ConfigureIdentityModel(this ModelBuilder modelBuilder)
    {
        ...
        var group = modelBuilder.Entity<ResourceGroupRow>(); group.ToTable("ResourceGroups"); group.HasKey(x => x.Id); group.HasIndex(x => new { x.TenantId, x.WorkspaceId, x.Name }).IsUnique(); group.HasOne<WorkspaceRow>().WithMany().HasForeignKey(x => x.WorkspaceId).OnDelete(DeleteBehavior.Cascade);
...
internal static class SqliteIdentitySchema
{
    public static async Task EnsureAsync(ControlPlaneDbContext context, CancellationToken cancellationToken)
    {
        ...
        await AddColumnIfMissingAsync(connection, "ControlPlaneResources", "ResourceGroupId", "TEXT NULL", cancellationToken);
        string[] commands =
        [
            ...
            "CREATE TABLE IF NOT EXISTS ResourceGroups (Id TEXT NOT NULL PRIMARY KEY, TenantId TEXT NOT NULL, WorkspaceId TEXT NOT NULL, Name TEXT NOT NULL, CreatedAt TEXT NOT NULL, FOREIGN KEY(WorkspaceId) REFERENCES Workspaces(Id) ON DELETE CASCADE); CREATE UNIQUE INDEX IF NOT EXISTS IX_ResourceGroups_TenantId_WorkspaceId_Name ON ResourceGroups(TenantId, WorkspaceId, Name);",
            ...
            "CREATE INDEX IF NOT EXISTS IX_ControlPlaneResources_Scope ON ControlPlaneResources(TenantId, WorkspaceId, ResourceType, ResourceGroup, ResourceId);"
        ];
...
public sealed class SqliteIdentityStore(IDbContextFactory<ControlPlaneDbContext> contextFactory) : IIdentityStore
{
    ...
    public async Task<ResourceGroup?> FindResourceGroupAsync(Guid tenantId,Guid workspaceId,string name,CancellationToken token){await using var db=await CreateAsync(token);return Map(await db.ResourceGroups.AsNoTracking().SingleOrDefaultAsync(x=>x.TenantId==tenantId&&x.WorkspaceId==workspaceId&&x.Name==name,token));}
    public Task AddResourceGroupAsync(ResourceGroup x,CancellationToken token)=>AddAsync(db=>db.ResourceGroups.Add(new(){Id=x.Id,TenantId=x.TenantId,WorkspaceId=x.WorkspaceId,Name=x.Name,CreatedAt=x.CreatedAt}),token);
    ...
    private static ResourceGroup? Map(ResourceGroupRow? x)=>x is null?null:new(x.Id,x.TenantId,x.WorkspaceId,x.Name,x.CreatedAt);
