...
public sealed class AepClient(HttpClient httpClient)
{
    public async Task<AepExtensionDescriptor> DiscoverAsync(CancellationToken cancellationToken = default)
    {
        using var response = await SendAsync(HttpMethod.Get, AepProtocol.DiscoveryPath, null, cancellationToken);
        var descriptor = await ReadAsync<AepExtensionDescriptor>(response, cancellationToken);
