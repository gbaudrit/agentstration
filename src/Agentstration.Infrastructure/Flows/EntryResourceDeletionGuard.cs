...
{
    public async Task ValidateDeleteAsync(string resourceId, CancellationToken cancellationToken)
    {
        ...
        var referenced = drafts.Where(value => publishedIds.Contains(value.Id) && string.Equals(value.PublishedBinding?.ResourceId, resourceId, StringComparison.Ordinal)).Select(value => value.Name).ToArray();
        if (referenced.Length > 0)
            throw new AgentDefinitionValidationException("resource_in_use", $"Resource '{resourceId}' is referenced by Entry: {string.Join(", ", referenced)}.");
    }
