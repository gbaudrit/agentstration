...
public sealed class FileSystemArtifactStore(string rootPath) : IArtifactStore
{
    ...
    public async Task<ArtifactReference> SaveAsync(WorkspaceId workspaceId, ArtifactContent content, CancellationToken cancellationToken)
    {
        ...
        var path = Resolve(workspaceId, key, createWorkspace: true);
    ...
    public Task<Stream> OpenReadAsync(WorkspaceId workspaceId, ArtifactReference reference, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        var stream = new FileStream(Resolve(workspaceId, reference.StorageKey), FileMode.Open, FileAccess.Read, FileShare.Read, 81920, FileOptions.Asynchronous | FileOptions.SequentialScan);
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        var path = Resolve(workspaceId, reference.StorageKey);
        if (File.Exists(path)) File.Delete(path);
        return Task.CompletedTask;
    }

    private string Resolve(WorkspaceId workspaceId, string key, bool createWorkspace = false)
    {
        ...
        if (!workspaceRoot.StartsWith(root + Path.DirectorySeparatorChar, StringComparison.OrdinalIgnoreCase))
            throw new WorkValidationException("artifact_path_invalid", "The workspace artifact path is outside the configured store.");
        if (createWorkspace) Directory.CreateDirectory(workspaceRoot);
        var path = Path.GetFullPath(Path.Combine(workspaceRoot, key));
