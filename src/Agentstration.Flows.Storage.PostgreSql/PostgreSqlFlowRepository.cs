...
namespace Agentstration.Flows.Storage.PostgreSql;
...
public sealed class PostgreSqlFlowRepository(IDbContextFactory<FlowDbContext> contextFactory, TimeProvider timeProvider) : IFlowRepository
{
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken)
    {
        ...
        if (!await context.Database.CanConnectAsync(cancellationToken)) throw new InvalidOperationException("The PostgreSQL Flow store is not accessible.");
    }
...
public static class PostgreSqlFlowServiceCollectionExtensions
{
    public static IServiceCollection AddPostgreSqlFlowStorage(this IServiceCollection services, string connectionString)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(connectionString);
        services.AddDbContextFactory<FlowDbContext>(options => options.UseNpgsql(connectionString, npgsql => npgsql.MigrationsHistoryTable("__EFMigrationsHistory", "flow").EnableRetryOnFailure()).AddInterceptors(new UtcDateTimeOffsetInterceptor()));
        services.AddSingleton<IFlowRepository, PostgreSqlFlowRepository>();
        return services;
    }
}
