...
internal sealed partial class TriggerEndpoints
{
    private static Task<IResult> PreviewScheduleAsync(
        ...
        TimeProvider timeProvider) => TriggersApiHttp.ExecuteAsync(() =>
        {
    ...
    private static Task<IResult> ListCoreAsync(ResourceNamespace? @namespace, TriggerManagementService service, CancellationToken token) => TriggersApiHttp.ExecuteAsync(async () =>
    {
    ...
    private static Task<IResult> GetCoreAsync(ResourceNamespace @namespace, string name, HttpResponse response, TriggerManagementService service, CancellationToken token) => TriggersApiHttp.ExecuteAsync(async () =>
    {
        ...
        return TriggersApiHttp.ResourceResult(stored, response, StatusCodes.Status200OK);
    });
    ...
    private static Task<IResult> PutCoreAsync(ResourceNamespace @namespace, string name, TriggerResource body, HttpRequest request, HttpResponse response, TriggerManagementService service, CancellationToken token) => TriggersApiHttp.ExecuteAsync(async () =>
    {
        TriggersApiHttp.RequireApiVersion(request);
        ...
        {
            var ifMatch = TriggersApiHttp.IfMatch(request) ?? throw new ResourceConcurrencyException("Updating a Trigger requires If-Match.");
            stored = await service.UpdateAsync(@namespace, name, body.Definition, ifMatch, token);
        }
        return TriggersApiHttp.ResourceResult(stored, response, current is null ? StatusCodes.Status201Created : StatusCodes.Status200OK);
    });
    ...
    private static Task<IResult> DeleteCoreAsync(ResourceNamespace @namespace, string name, HttpRequest request, TriggerManagementService service, CancellationToken token) => TriggersApiHttp.ExecuteAsync(async () =>
    {
        var ifMatch = TriggersApiHttp.IfMatch(request) ?? throw new ResourceConcurrencyException("Deleting a Trigger requires If-Match.");
    ...
    private static Task<IResult> RunNowCoreAsync(ResourceNamespace @namespace, string name, ICurrentRequestContext context, IAuthorizationService authorization, TriggerFiringService service, CancellationToken token) => TriggersApiHttp.ExecuteAsync(async () =>
    {
    ...
    private static Task<IResult> HistoryCoreAsync(ResourceNamespace @namespace, string name, int? take, TriggerManagementService management, TriggerFiringService firing, CancellationToken token) => TriggersApiHttp.ExecuteAsync(async () =>
    {
