...
public static class WorkOperationsEndpoints
{
    ...
    private static Task<IResult> ListAsync(
        ...
        WorkplaceService service, FlowRunService flowRuns, CancellationToken token) => ExecuteAsync(async () =>
    {
        ...
        var result = await service.QueryOperationalTasksAsync(workspaceId, status, search, hasPendingAction, actualPage, actualPageSize, sortField, sortDirection, token);
        var summaries = new List<WorkTaskOperationsSummary>(result.Items.Count);
        foreach (var task in result.Items) summaries.Add(await SummaryOfAsync(task, service, flowRuns, token));
        return Results.Ok(new WorkTaskOperationsPageResponse(summaries, actualPage, actualPageSize, result.TotalCount));
    });

    private static Task<IResult> SummaryAsync(string? workspaceId, WorkplaceService service, FlowRunService flowRuns, TimeProvider timeProvider, CancellationToken token) => ExecuteAsync(async () =>
    {
        async Task<int> Count(WorkTaskStatus status, DateTimeOffset? updatedFrom = null) =>
            (await service.QueryOperationalTasksAsync(workspaceId, status, null, null, 1, 1, WorkItemSortField.UpdatedAt, WorkItemSortDirection.Descending, token, updatedFrom)).TotalCount;
    ...
    private static Task<IResult> DetailAsync(Guid taskId, WorkplaceService service, FlowRunService flowRuns, CancellationToken token) => ExecuteAsync(async () =>
    {
        var operational = await service.GetOperationalTaskAsync(new(taskId), token); var task = operational.Task;
        ...
        var runs = await RunsForAsync(task.Id, flowRuns, results, artifacts, token);
        return Results.Ok(new WorkTaskOperationsDetailResponse(
            await SummaryOfAsync(task, service, flowRuns, token), ToInteraction(interaction), pending, runs,
    ...
    private static Task<IResult> ActivitiesAsync(Guid taskId, WorkplaceService service, CancellationToken token) => WithTaskAsync(taskId, service, async value => Results.Ok(await service.ListActivitiesAsync(value.WorkspaceId, value.Task.Id, token)), token);
    private static Task<IResult> PendingActionsAsync(Guid taskId, WorkplaceService service, CancellationToken token) => WithTaskAsync(taskId, service, async value => Results.Ok((await service.ListPendingActionsAsync(value.WorkspaceId, value.Task.InteractionId, token)).Where(action => action.WorkTaskId == value.Task.Id).Select(WorkplaceService.ToContract)), token);
    private static Task<IResult> ResultsAsync(Guid taskId, WorkplaceService service, CancellationToken token) => WithTaskAsync(taskId, service, async value => Results.Ok((await service.ListResultsAsync(value.WorkspaceId, value.Task.Id, token)).Select(ToResult)), token);
    private static Task<IResult> ArtifactsAsync(Guid taskId, WorkplaceService service, CancellationToken token) => WithTaskAsync(taskId, service, async value => Results.Ok((await service.ListArtifactsAsync(value.WorkspaceId, value.Task.Id, token)).Select(artifact => ToArtifact(artifact, value.WorkspaceId))), token);
    private static Task<IResult> FlowRunsAsync(Guid taskId, WorkplaceService service, FlowRunService flowRuns, CancellationToken token) => WithTaskAsync(taskId, service, async value =>
    {
        ...
        return Results.Ok(await RunsForAsync(value.Task.Id, flowRuns, results, artifacts, token));
    }, token);
    private static Task<IResult> FlowRunAsync(Guid taskId, string runId, WorkplaceService service, FlowRunService flowRuns, CancellationToken token) => WithTaskAsync(taskId, service, async value =>
    {
        var run = await flowRuns.GetAsync(runId, token);
    ...
    private static Task<IResult> PauseAsync(Guid taskId, WorkplaceService service, CancellationToken token) => WithTaskAsync(taskId, service, async value => Results.Ok(await service.PauseTaskAsync(value.WorkspaceId, value.Task.Id, token)), token);
    ...
    private static async Task<IReadOnlyList<WorkTaskFlowRunResponse>> RunsForAsync(WorkTaskId taskId, FlowRunService service, IReadOnlyList<WorkTaskResult> results, IReadOnlyList<WorkTaskArtifact> artifacts, CancellationToken token)
    {
        var page = await service.ListAsync(null, null, 0, 200, token);
