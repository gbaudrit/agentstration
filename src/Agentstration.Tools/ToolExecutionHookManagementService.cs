...
public sealed class ToolExecutionHookManagementService(IResourceStore store)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (await GetAsync(resource.Namespace, resource.Name, cancellationToken) is not null)
            throw new ResourceConcurrencyException($"Tool execution hook '{resource.Address}' already exists.");
    ...
    public Task<StoredResource<ToolExecutionHookResource>?> GetAsync(
        ...
        store.GetAsync<ToolExecutionHookResource>(
            new ResourceKey(ToolResourceKinds.ToolExecutionHook, name, @namespace),
            cancellationToken);

    public Task<IReadOnlyList<StoredResource<ToolExecutionHookResource>>> ListAsync(CancellationToken cancellationToken) =>
        store.ListAllAsync<ToolExecutionHookResource>(ToolResourceKinds.ToolExecutionHook, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        var existing = await GetAsync(@namespace, name, cancellationToken)
            ?? throw new ResourceNotFoundException(new(ToolResourceKinds.ToolExecutionHook, name, @namespace));
        ...
        CancellationToken cancellationToken)
    {
        _ = await GetAsync(@namespace, name, cancellationToken)
            ?? throw new ResourceNotFoundException(new(ToolResourceKinds.ToolExecutionHook, name, @namespace));
        await store.DeleteAsync(new ResourceKey(ToolResourceKinds.ToolExecutionHook, name, @namespace), ifMatch, cancellationToken);
    }

    public static void Validate(ToolExecutionHookResource resource)
    {
        if (resource.Kind != ToolResourceKinds.ToolExecutionHook)
            throw new ToolExecutionHookValidationException($"Kind must be '{ToolResourceKinds.ToolExecutionHook}'.");
        if (resource.ApiVersion != ResourceApiVersions.CoreV1)
            throw new ToolExecutionHookValidationException($"ApiVersion must be '{ResourceApiVersions.CoreV1}'.");
