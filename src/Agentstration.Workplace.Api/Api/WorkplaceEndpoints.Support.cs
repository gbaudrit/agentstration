...
{
    private static WorkAttachment ToWorkAttachment(WorkAttachmentRequest request)
    {
        if (string.IsNullOrWhiteSpace(request.Name))
            throw new WorkValidationException("attachment_name_required", "An attachment name is required.");
        if (!Uri.TryCreate(request.Uri, UriKind.Absolute, out _))
            throw new WorkValidationException("attachment_uri_invalid", "An attachment must reference an absolute URI.");
        return new WorkAttachment(
            request.Name.Trim(),
            new WorkContentReference(request.Uri, request.MediaType, request.Name),
            request.Size,
            request.Metadata);
    }
