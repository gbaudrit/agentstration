...
public sealed class ToolsApiClient(HttpClient httpClient) : IToolsClient
{
    ...
    {
        using var response = await httpClient.GetAsync(ToolPath(name), cancellationToken);
    ...
    {
        using var message = new HttpRequestMessage(HttpMethod.Put, ToolPath(name) + "/enabled") { Content = JsonContent.Create(new SetToolEnabledRequest(enabled)) };
    ...
    private static string ToolPath(string name) => $"api/tools/{Uri.EscapeDataString(name)}";
}
