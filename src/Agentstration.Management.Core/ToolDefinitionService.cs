    ...
    TimeProvider timeProvider)
{
    ...
    public Task<StoredResource<ToolDefinitionResource>?> GetAsync(
        ...
        store.GetAsync<ToolDefinitionResource>(new ResourceKey(ResourceKinds.ToolDefinition, name, @namespace), cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        var current = await GetAsync(name, @namespace, cancellationToken)
            ?? throw new ControlPlaneResourceNotFoundException(new(ResourceKinds.ToolDefinition, name, @namespace));
    ...
    public async Task DeleteAsync(string name, ResourceNamespace @namespace, string? ifMatch, CancellationToken cancellationToken)
    {
        var current = await GetAsync(name, @namespace, cancellationToken)
            ?? throw new ControlPlaneResourceNotFoundException(new(ResourceKinds.ToolDefinition, name, @namespace));
        var tool = new ResourceKey(ResourceKinds.Tool, AgentstrationToolProvider.ToolResourceName(name), @namespace);
        ...
        await store.DeleteAsync(new ResourceKey(ResourceKinds.ToolDefinition, name, @namespace), ifMatch ?? current.ETag, cancellationToken);
    }
    ...
    {
        if (resource.Kind != ResourceKinds.ToolDefinition || resource.ApiVersion != ManagementApiVersions.CoreV1)
    ...
    {
        var key = new ResourceKey(ResourceKinds.ToolProvider, AgentstrationToolProvider.Name, @namespace);
    ...
    private async Task MaterializeAsync(ToolDefinitionResource definition, CancellationToken cancellationToken)
    {
        ...
        var key = new ResourceKey(ResourceKinds.Tool, name, definition.Namespace);
