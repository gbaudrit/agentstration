...
public sealed partial class PackManagementService
{
    ...
    private async Task<bool> BindingTargetExistsAsync(PackBindingTargetKind kind, ResourceReference target, CancellationToken cancellationToken)
    {
        ...
        return kind switch
        {
            ...
            PackBindingTargetKind.MemoryProfile => await store.GetAsync<MemoryProfileResource>(new(ResourceKinds.MemoryProfile, target.Name, @namespace), cancellationToken) is not null,
            PackBindingTargetKind.MemoryProvider => await store.GetAsync<MemoryProviderResource>(new(ResourceKinds.MemoryProvider, target.Name, @namespace), cancellationToken) is not null,
