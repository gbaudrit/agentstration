...
public sealed class PostgreSqlWorkItemRepository(IDbContextFactory<WorkDbContext> contextFactory) : IWorkItemRepository
{
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken)
    {
        ...
        if (!await context.Database.CanConnectAsync(cancellationToken)) throw new InvalidOperationException("The PostgreSQL Work store is not accessible.");
    }
...
public sealed class PostgreSqlWorkplaceRepository(IDbContextFactory<WorkDbContext> contextFactory) : IWorkplaceRepository
{
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);
...
public static class PostgreSqlWorkServiceCollectionExtensions
{
    public static IServiceCollection AddPostgreSqlWorkPlane(this IServiceCollection services, string connectionString)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(connectionString);
        services.AddDbContextFactory<WorkDbContext>(options => options.UseNpgsql(connectionString, npgsql => npgsql.MigrationsHistoryTable("__EFMigrationsHistory", "work").EnableRetryOnFailure()).AddInterceptors(new UtcDateTimeOffsetInterceptor()));
        services.AddSingleton<IWorkItemRepository, PostgreSqlWorkItemRepository>();
        services.AddSingleton<IWorkplaceRepository, PostgreSqlWorkplaceRepository>();
        return services;
    }
}
