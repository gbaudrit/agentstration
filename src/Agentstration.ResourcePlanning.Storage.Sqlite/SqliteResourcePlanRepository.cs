    ...
    IDbContextFactory<ResourcePlanningDbContext> contextFactory) : IResourcePlanRepository, IResourceChangeSetRepository
{
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken)
    {
        ...
        await context.Database.ExecuteSqlRawAsync("""
            CREATE TABLE IF NOT EXISTS ResourcePlanBindingDrafts (
                TenantId TEXT NOT NULL, WorkspaceId TEXT NOT NULL, PlanId TEXT NOT NULL,
                Payload TEXT NOT NULL, ETag TEXT NOT NULL,
                CONSTRAINT PK_ResourcePlanBindingDrafts PRIMARY KEY (WorkspaceId, PlanId)
            )
            """, cancellationToken);
        await context.Database.ExecuteSqlRawAsync("""
            CREATE TABLE IF NOT EXISTS ResourceChangeSetApplications (
                TenantId TEXT NOT NULL, WorkspaceId TEXT NOT NULL, ChangeSetId TEXT NOT NULL,
                Payload TEXT NOT NULL, ETag TEXT NOT NULL,
                CONSTRAINT PK_ResourceChangeSetApplications PRIMARY KEY (WorkspaceId, ChangeSetId)
            )
            """, cancellationToken);
    }
