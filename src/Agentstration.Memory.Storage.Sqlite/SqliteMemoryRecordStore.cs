...
public sealed class SqliteMemoryRecordStore(IDbContextFactory<MemoryDbContext> contexts) : IMemoryRecordStore
{
    ...
    public async Task<int> PurgeExpiredAsync(DateTimeOffset now, int take, CancellationToken cancellationToken)
    {
        await using var context = await contexts.CreateDbContextAsync(cancellationToken);
        var ids = await context.Records.Where(value => value.ExpiresAt != null && value.ExpiresAt <= now.UtcTicks).OrderBy(value => value.ExpiresAt).Take(take).Select(value => new { value.WorkspaceId, value.Id }).ToArrayAsync(cancellationToken);
        var deleted = 0;
        foreach (var id in ids) deleted += await context.Records.Where(value => value.WorkspaceId == id.WorkspaceId && value.Id == id.Id).ExecuteDeleteAsync(cancellationToken);
        return deleted;
    }
