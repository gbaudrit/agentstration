...
For a Model Provider whose discovery requires a bound Secret, the host uses authenticated `POST /aep/model-providers/{providerId}/models/discover` with an `AepModelDiscoveryRequest` containing bounded `secretAccess` grants. The extension redeems each grant during that discovery and the host revokes any unused grant when the call ends. Unbound discovery retains `GET /aep/model-providers/{providerId}/models`. The discovery request and its grant fields must receive the same trace redaction as chat requests.
