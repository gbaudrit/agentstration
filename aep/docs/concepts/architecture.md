...
Source Provider contributions use the canonical AEP client directly. They resolve provider-specific mutable selectors into immutable revisions and materialize only exact revisions as bounded opaque archives. The host application, never the extension, interprets the archive and owns Source and catalog semantics.
