...
Aspire starts Agentstration's local AEP extensions against existing inference servers. The Microsoft Foundry extension is opt-in through `Foundry:Enabled=true` and AppHost User Secrets; it supports deployment discovery, text chat, streaming and governed Tool calls for local developer tests. See [Foundry integration](docs/foundry-integration.md#opt-in-aspire-launch-for-local-tests). Provider-specific Compose files isolate Ollama, llama.cpp, and LocalAI, with each topology owning its inference service and model storage. No topology downloads a model implicitly. Follow the [local installation guide](https://docs.agentstration.io/getting-started/local-installation) and [model provider guide](https://docs.agentstration.io/concepts/model-providers) for provider-specific setup.
